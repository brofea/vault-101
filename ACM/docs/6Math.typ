...
$
integral k dif x &= k x + C \
integral x^n dif x &= frac(x^(n+1), n+1) + C quad (n != -1) \
integral frac(1, x) dif x &= ln |x| + C \
integral e^x dif x &= e^x + C \
integral a^x dif x &= frac(a^x, ln a) + C \
$
$
...
游戏先手必败，当且仅当奇数堆石子数量的 Nim 和 $a_1 xor a_3 xor dots = 0$
...
不妨设 $a_1 <= a_2$，那么，先手必败，当且仅当 $a_1 = floor((a_2-a_1) phi)$，其中 $phi = (sqrt(5)+1)/2$ 是黄金分割比。

=== 反常 Nim 游戏 (Misere Play)
...
- 存在至少一堆石子数 $> 1$，且 Nim 和为 0
- 所有堆石子数都 $<= 1$（也就是每堆要么 0 要么 1），并且非空堆的个数是奇数

否则就是先手必胜局面。

== 计算几何

点 $(x,y)$ 到直线 $A x+B y+C=0$ 的距离为：

$ d=(|A x+B y+C|) / sqrt(A^2+B^2) $

直线 $A x+B y+C_1=0$ 和 $A x+B y+C_2=0$ 之间的距离为：
...
$ p = (a+b+c)/2 comma S = sqrt(p(p-a)(p-b)(p-c)) $
